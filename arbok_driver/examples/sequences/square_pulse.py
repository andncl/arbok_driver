...
class SquarePulse(SubSequence):
    ...
    Demonstrates backend-specific override: fpga_sequence__qua() contains
    raw QUA SDK calls and only runs when QuaBackend is active.
    """
    PARAMETER_CLASS = SquarePulseParameters
    arbok_params: SquarePulseParameters

    def fpga_sequence__qua(self):
        """QUA-backend-specific implementation using raw QUA SDK."""
        qua.align()
        qua.play(
